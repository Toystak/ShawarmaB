...
global.authFile = `ShawarmaSession`;
  ...
  if (opcion === "2" || methodCode) {
    ...
    if (!conn.authState.creds.registered) {
      ...
      setTimeout(async () => {
        ...
        console.log(
          chalk.black(chalk.bgGreen(`Su código de emparejamiento: `)),
          chalk.black(chalk.white(codigo)),
        );
      }, 3000);
    }
  }
...
function purgeSession() {
  let prekey = [];
  let directorio = readdirSync("./ShawarmaSession");
  let filesFolderPreKeys = directorio.filter((file) => {
    return file.startsWith(
      "pre-key-",
    ); /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
  });
  prekey = [...prekey, ...filesFolderPreKeys];
  filesFolderPreKeys.forEach((files) => {
    unlinkSync(`./ShawarmaSession/${files}`);
  });
}
  ...
  try {
    ...
    listaDirectorios.forEach((directorio) => {
      if (statSync(`./jadibts/${directorio}`).isDirectory()) {
        let DSBPreKeys = readdirSync(`./jadibts/${directorio}`).filter(
          (fileInDir) => {
            return fileInDir.startsWith(
              "pre-key-",
            ); /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
          },
        );
    ...
    console.log(
      chalk.bold.red(
        `[ ℹ️ ] Algo salio mal durante la eliminación, archivos no eliminados`,
      ),
    );
  }
}
...
function purgeOldFiles() {
  const directories = ["./ShawarmaSession/", "./jadibts/"];
        ...
        stat(filePath, (err, stats) => {
          ...
          if (
            ...
            unlinkSync(filePath, (err) => {
              if (err) throw err;
              console.log(
                chalk.bold.green(`Archivo ${file} borrado con éxito`),
              );
            });
          } else {
...
async function connectionUpdate(update) {
  ...
  if (reason == 405) {
    await fs.unlinkSync("./ShawarmaSession/" + "creds.json");
    console.log(
      chalk.bold.redBright(
  ...
  if (connection === "close") {
    if (reason === DisconnectReason.badSession) {
      conn.logger.error(
        `[ ⚠ ] Sesión incorrecta, por favor elimina la carpeta ${global.authFile} y escanea nuevamente.`,
      );
      //process.exit();
      ...
      conn.logger.error(
        `[ ⚠ ] Conexión reemplazada, se ha abierto otra nueva sesión. Por favor, cierra la sesión actual primero.`,
      );
      //process.exit();
    } else if (reason === DisconnectReason.loggedOut) {
      conn.logger.error(
        `[ ⚠ ] Conexion cerrada, por favor elimina la carpeta ${global.authFile} y escanea nuevamente.`,
      );
      //process.exit();
    } else if (reason === DisconnectReason.restartRequired) {
      conn.logger.info(
        `[ ⚠ ] Reinicio necesario, reinicie el servidor si presenta algún problema.`,
      );
      await global.reloadHandler(true).catch(console.error);
      ...
      conn.logger.warn(
        `[ ⚠ ] Razón de desconexión desconocida. ${reason || ""}: ${connection || ""}`,
      );
      await global.reloadHandler(true).catch(console.error);
    }
  }
